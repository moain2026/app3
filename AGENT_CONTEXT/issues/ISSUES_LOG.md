...
## ISS-14 — فجوات المطابقة البصرية/السلوكية من صور التطبيق الأصلي
- **المصدر:** صور رسمية + تقرير PDF حقيقي (ارصدة الحسابات، 42 صفحة، 2026-05-29)
  من المالك. موثّق بالكامل في `app1_legacy_context/UI_GROUND_TRUTH.md`.
- **الفجوات المكتشفة:**
  1. **الرئيسية (Home):** الأصل = مشغّل بتبويبين فقط: **عمليات** (سندات القبض،
     القراءات) و**تقارير** (6 تقارير). **ماكو "عدادات اليوم"**. تطبيقنا يعرض شبكة
     KPI (قراءات اليوم/سندات اليوم/معلّق/آخر مزامنة) = اختراع غير موجود بالأصل.
  2. **اصطلاح إشارة الرصيد:** موجب=عليه(مدين، أحمر)، سالب=له(دائن، أزرق). التقارير
     لازم تلوّن وتسمّي عبر الإعدادات (دائن=له، مدين=عليه). إجمالي الـ PDF=-1,748,065.
  3. **رقم العداد (meter serial) = STRING** بطول متغيّر حتى 13 رقم (مثال
     2022870003191) — ممنوع تخزينه int.
  4. **القراءة الحالية تساوي السابقة** افتراضياً حتى تُعدّل.
  5. **قائمة سندات القبض:** أعمدة التاريخ/المبلغ/الحساب/الرقم/البيان؛ الرقم تسلسلي
     (3122..3126)، صيغة التاريخ المعروضة yyyy/M/d.
  6. **الإعدادات:** نص الرسالة + نهايتها، عدد أيام عرض المستندات، تسمية حالة الحساب،
     الطابعة (XPrinter/58mm/عادي) — كلها config لازم تكون موجودة بإعدادات app3.
     **تنبيه:** طابعة الأصل بهذا الحساب = XPrinter 58mm مو BIXOLON (النوع قابل للاختيار).
  7. **تصدير PDF** من شاشة الأرصدة (أيقونة PDF برتقالية) ← تقرير 42 صفحة بترويسة/
     تذييل دقيق + اعتماد `YDsoft`.
- **الحالة:** موثّق؛ بانتظار قرار المالك بخصوص إعادة تصميم الرئيسية (تغيير كبير).
- **مرجع:** `app1_legacy_context/UI_GROUND_TRUTH.md`.
